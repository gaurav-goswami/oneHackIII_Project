...
def create_meeting(user_input: str):
    ...
    prompt = (
        """using the following format please return ONLY a dictionary that represents a Zoom meeting that's described below.  do not say anything other than the dictionary, some keys are optional.  indicate the time zones using the formats below as well, do not use any other format for the time zone."""
        + user_input
        + request_string
    )
